-- cria a função set_updated_at se não existir (usa bigdata_final)
DO $do$ 
BEGIN 
    IF NOT EXISTS ( 
        SELECT 1 
        ...
        AND n.nspname = 'bigdata_final' 
    ) THEN 
        EXECUTE '
            CREATE FUNCTION bigdata_final.set_updated_at() 
            ...
            BEGIN 
                NEW.updated_at = now(); 
                arquivo := ''/var/lib/postgresql/data/export/'' || TG_TABLE_NAME || ''.'' || NEW.cpf || ''.json''; 
...
-- cria a função auxiliar create_updated_at_trigger se não existir (no mesmo bigdata_final)
DO $do$
BEGIN
    IF NOT EXISTS (
        SELECT 1
        ...
        EXECUTE $fn$
            CREATE FUNCTION bigdata_final.create_updated_at_trigger(p_schema text, p_table text)
...
-- exemplo: cria triggers para várias tabelas (substitua bigdata_final)
DO $do$
DECLARE
    tbls text[] := ARRAY[
...
BEGIN
    ...
    LOOP
        PERFORM bigdata_final.create_updated_at_trigger('bigdata_final', t);
    END LOOP;
END;
$do$;
